...
insert into public.clientes (nome, setor, endereco, observacoes, created_by) values
  ('Metalúrgica Vale Forte S.A.', 'Siderúrgico', 'Av. das Indústrias, 1200 - Cubatão/SP', 'Cliente recorrente, negocia contratos fixos anuais.', 'fec22346-6f75-4a0f-96e5-91b1594aff44'),
...
) as v(numero_proposta, data_envio, cliente_nome, servico, descricao, valor, status_key, termometro, tipo_servico)
join public.clientes c on c.nome = v.cliente_nome
...
from (values
  ('012/26', 'Reunião de alinhamento realizada em 14/01 — cliente aprovou escopo.'),
  ('014/26', 'Aguardando retorno do jurídico do cliente sobre cláusulas de NR-37.'),
  ('017/26.1', 'Cliente pediu revisão de valores após aumento de volume mensal.')
) as h(numero_proposta, texto)
join public.propostas p on p.numero_proposta = h.numero_proposta;

insert into public.interacoes_cliente (cliente_id, autor_id, tipo, descricao, data_interacao)
...
from (values
  ...
  ('AutoPeças Rondelli Ltda', 'follow_up', 'Follow-up sobre proposta de milk-run, cliente pediu ajuste de rota.', '2026-02-25 09:00:00-03')
) as i(cliente_nome, tipo, descricao, data_interacao)
join public.clientes c on c.nome = i.cliente_nome;

insert into public.compromissos (titulo, descricao, inicio, fim, cliente_id, criado_por)
...
) as cp(titulo, descricao, inicio, fim, cliente_nome)
join public.clientes c on c.nome = cp.cliente_nome;
